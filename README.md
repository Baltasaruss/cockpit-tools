...
本仓库基于 [jlcodes99/cockpit-tools](https://github.com/jlcodes99/cockpit-tools) 继续开发，当前分支主要增加了以下能力：

- **Antigravity CLI 支持**：可将 Antigravity 的运行目标设为 CLI，并在切换账号时只更新 CLI 身份，不额外启动 IDE 或终端。
- **macOS 菜单栏多平台额度**：最多选择 3 个平台，以图标和剩余额度同时展示；顺序跟随侧边栏平台布局，并支持彩色或单色数值。
- **原生额度弹窗优化**：平台选项跟随用户当前的平台布局，统一使用 `Grok CLI`、`Agy CLI` 命名，并记住上次选中的平台标签。
- **菜单栏图标优化**：补充 Grok 与 Antigravity 图标，使用适配 macOS 明暗状态栏的模板图标。
- **Grok CLI 切号修正**：切换后写入当前账号凭据，额度展示与当前账号保持一致。

除上述增强外，其余功能与使用方式延续上游项目。
