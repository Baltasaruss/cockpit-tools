...
- **Codex OAuth Token 支持手动强制刷新**：账号总览可使用当前 `refresh_token` 获取新的 `access_token` 与 `id_token`，刷新结果会写回本地账号并立即更新页面状态；结果、失败原因、重新检测和重新授权均在独立弹框内完成。
- **Codex 账号池增加池级异常诊断**：当请求没有选出可用账号时，账号池异常弹框会展示模型、候选账号、授权身份、代理选路、范围匹配、可用性、额度保留和生图策略等诊断信息，并支持重新同步账号池。
- **API Service 增加按账号的生图策略**：账号池可对指定账号单独设置生图自动、启用或禁用策略，文本请求与生图请求按实际账号能力分别路由。

### 变更

- **Codex 不再将 `refresh_token_reused` 作为自动账号异常或切换条件**：自动流程不会因该返回结果阻止切号、启动或写入账号失败状态；启动预览仍支持手动强制刷新 Token，刷新失败仅在当前弹框中反馈。
- **Codex 启动进度支持随时取消及失败后跳过启动**：启动过程中可取消事务或通过关闭同时取消并关闭弹框；可跳过的检查、网关准备和维护步骤失败后，可在当前弹框中选择“跳过并启动”继续启动客户端。
- **Codex 客户端认证观测降低运行时开销**：实时登录状态使用轻量 CDP 页面快照每 5 秒检查；完整网络诊断改为每 30 秒后台采集一次，不再阻塞状态更新或持续占用客户端调试通道。
- **Codex 客户端重新授权提示改为说明登录页状态**：用户可重新授权恢复客户端使用；如果暂时只使用 API 服务，仍可继续使用。
- **Codex 客户端需授权状态仅作为账号卡片提示**：该观测状态不再限制切号或启动；详情弹框提供重新授权和清除异常标识操作。
- **Codex 客户端状态与 API 授权状态分级展示**：客户端登录页观测、Token 刷新异常和服务端撤销不再混用同一状态；服务端明确撤销时按最高级授权异常处理，不再同时显示“客户端需授权”或“API 服务仍可用”。
...
- **Codex 额度刷新降低重复运行态探测**：同一批账号复用一次进程快照并限制并发额度请求，减少 Windows 子进程开销和高峰期请求争用。
- **OAuth 授权链接的客户端版本支持远端默认值与本地缓存**：授权链接使用固定的官方客户端版本；默认版本可由远端配置调整并缓存到本地，远端暂时不可访问时仍可继续授权；用户也可在设置中手动覆盖。
- **Codex 浏览器 OAuth 授权等待时间延长至 10 分钟**：登录、MFA 和回调处理有更充足的完成时间；设备代码授权时限保持不变。
- **Codex 订阅信息明确标记为“订阅有效期”**：账号卡片和相关详情不再将订阅截止时间与 Token 有效期混用。

### 修复

- **修复取消或关闭 Codex 切号流程后账号卡片持续旋转**：取消事件会立即结束对应账号卡片的切换状态，后端事务仍可在安全检查点完成清理。
- **修复客户端登录状态被普通页面文字误判的问题**：客户端需授权现在仅在 Codex 官方 `/login` 路由，或桌面端 `MemoryRouter` 使用的完整 LoginRoute 界面特征确认后标记；普通页面中的登录相关文字、认证提示或其他 CDP 页面不会再触发账号重新授权状态。
- **修复客户端登录状态写入后账号卡片更新延迟**：官方客户端登录页或恢复可用状态经过连续确认并落盘后，账号总览会立即同步最新状态，无需手动刷新或等待下一轮轮询。
- **Codex 客户端登录页观测补充实例时间线**：异常详情按顺序展示实例启动时间、跳转登录页时间、检测时间和检测实例，便于判断客户端何时进入登录页。
- **修复客户端观测状态误阻断切号和启动**：客户端登录页观测不再被包装为启动授权失败；清除异常标识只删除本地观测字段，不会修改 Token 或真实的服务端授权异常。
- **修复 API Service 实际请求失败后账号池异常未及时显示**：内部调度层返回 `auth_unavailable` 时也会生成账号级和池级诊断并立即同步到页面；错误信息跟随 Cockpit Tools 语言设置，中文界面显示中文，其他语言显示英文。
- **修复 API 401 后切号弹框仍显示“API 服务可用”**：切号授权失败结果现在同时检查远端 API 拒绝记录和本地 Token 到期时间，实际不可用账号不会再显示 API-only 可用提示。
- **修复新授权或官方客户端轮换后的 Token 被旧数据覆盖**：切号、重新授权、额度刷新、本地同步和 API 绑定会保留最新凭据，避免恢复旧 `access_token` 或 `refresh_token` 后再次出现远端撤销。
- **修复第三方 API Service 账号池显示“无可用账号”但没有异常明细**：没有具体账号异常时也会保留池级失败记录，并在账号池弹框中给出可操作的恢复入口。
- **修复 OAuth 绑定 API Key/API Service 后授权状态与实际凭据不同步**：重新授权、解绑、改绑和多开实例启动后的最新凭据会同步到对应 profile，不再继续复用旧授权状态。
- **修复 API Service 文本请求被生图能力错误阻断**：文本、编辑和生图请求按请求类型及账号策略处理，纯文本请求不会因上游缺少生图能力而失败。
- **修复部分页面按钮回退为浏览器原生灰色样式**：相关按钮、禁用态、弹框操作区和窄宽度文案统一使用项目视觉系统，保持既有字体、字号和布局。
